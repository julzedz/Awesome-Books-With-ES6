...
const zt = ['AM', 'PM'];
const qt = ['Before Christ', 'Anno Domini'];
const At = ['BC', 'AD'];
const jt = ['B', 'A'];
function _t(t) {
  switch (t) {
...
const qe = /^-?P(?:(?:(-?\d{1,20}(?:\.\d{1,20})?)Y)?(?:(-?\d{1,20}(?:\.\d{1,20})?)M)?(?:(-?\d{1,20}(?:\.\d{1,20})?)W)?(?:(-?\d{1,20}(?:\.\d{1,20})?)D)?(?:T(?:(-?\d{1,20}(?:\.\d{1,20})?)H)?(?:(-?\d{1,20}(?:\.\d{1,20})?)M)?(?:(-?\d{1,20})(?:[.,](-?\d{1,20}))?S)?)?)$/;
function Ae(t) {
      ...
      (function (e) {
        ...
        if (isNaN(r)) {
          ...
          for (let t = 0; t < e.length; t++) {
            ...
            if (e[t].search(kr.hanidec) !== -1) r += Nr.indexOf(e[t]);
            else {
              for (const a in Mr) {
                const [s, i] = Mr[a];
                s <= n && n <= i && (r += n - s);
...
function jr(s, t) {
  ...
  const r = e.formatDateTimeParts((Lr = Lr || L.fromMillis(1555555555555)));
  return r.map((e) => {
    {
      const r = s;
...
function en(t) {
  return t.weekData === null && (t.weekData = Rr(t.c)), t.weekData;
}
function rn(t, e) {
  t = {
    ts: t.ts,
...
class L {
  ...
  toISODate({ format: t = 'extended' } = {}) {
    return this.isValid ? cn(this, t === 'extended') : null;
  }
  ...
  toISOTime({
    ...
    return this.isValid
      ? (n ? 'T' : '') + hn(this, i === 'extended', e, t, r, s)
      : null;
  }
